...
<html lang="en">
  ...
  <body>
    ...
    <div style="position: relative; top:60px; right:-150px;">
      <h1>Update Staffs Documents</h1>
      <form action="{{ url('/update_staffs',$data->id) }}" method="post" enctype="multipart/form-data">
        @csrf
       ...
       <div>
        <div>
          ...
          <img height="200" width="200" src="staffs/{{ $data->image }}" hidden >
      </div>
            <div>
              <label>File (pdf,word,images,zip):</label>
              <input type="file" name="image" value="{{ $data->image }}" >
          </div>
          <a href="{{ url('/admin_staffs') }}">
