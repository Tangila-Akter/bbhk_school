  ...
  <div class="modal-dialog">
    <div class="modal-content">
      ...
      <div class="modal-body">
          <form action="{{ url('/uploadnotice') }}" method="post" enctype="multipart/form-data">
              @csrf
...
<br>
<!--Notice View start-->
<form action="{{ url('noticesearch') }}" method="get" class="form-inline" style="float:right; padding:10px; color:rgb(255, 255, 255);">
  @csrf
    ...
    <tbody>
      <tr style="background-color: #ffffff; color:black;" class="item">
        <td>{{ $data->title }}</td>
             <td>{{ $data->date }}</td>
             <td>{{ $data->filename }}</td>
             <td><a class="btn btn-warning" href="{{ url('/deletenotice',$data->id) }}">Delete</a>&nbsp<a class="btn btn-primary" href="{{ url('/updatenotice',$data->id) }}">Edit</a></td> 
      </tr>
    </tbody>
    @endforeach
