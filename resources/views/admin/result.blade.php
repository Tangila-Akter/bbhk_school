  ...
  <div class="modal-dialog">
    <div class="modal-content">
      ...
      <div class="modal-body">
          <form action="{{ url('/uploadresult') }}" method="post" enctype="multipart/form-data">
              @csrf
...
<br>
<!--Notice View start-->
<form action="{{ url('resultsearch') }}" method="get" class="form-inline" style="float:right; padding:10px; color:rgb(255, 255, 255);">
  @csrf
    ...
    <tbody>
      <tr style="background-color: #ffffff; color:black;" class="item">
        <td>{{ $data->title }}</td>
             <td>{{ $data->date }}</td>
             <td>{{ $data->image }}</td>
             <td><a class="btn btn-warning" href="{{ url('/deleteresult',$data->id) }}">Delete</a>&nbsp<a class="btn btn-primary" href="{{ url('/updateresult',$data->id) }}">Edit</a></td> 
      </tr>
    </tbody>
    @endforeach
