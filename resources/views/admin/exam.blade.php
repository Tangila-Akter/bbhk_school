  ...
  <div class="modal-dialog">
    <div class="modal-content">
      <div class="modal-header">
        <h5 class="modal-title" id="exampleModalLabel">Add Exam Routine</h5>
        <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
      </div>
      <div class="modal-body">
          <form action="{{ url('/uploadexam') }}" method="post" enctype="multipart/form-data">
              @csrf
...
<br>
<!--Notice View start-->
<form action="{{ url('examsearch') }}" method="get" class="form-inline" style="float:right; padding:10px; color:rgb(255, 255, 255);">
  @csrf
    ...
    <tbody>
      <tr style="background-color: #ffffff; color:black;" class="item">
        <td>{{ $data->title }}</td>
             <td>{{ $data->date }}</td>
             <td>{{ $data->filename }}</td>
             <td><a class="btn btn-warning" href="{{ url('/deleteexam',$data->id) }}">Delete</a>&nbsp<a class="btn btn-primary" href="{{ url('/updateexam',$data->id) }}">Edit</a></td> 
      </tr>
    </tbody>
    @endforeach
